# source: https://raw.githubusercontent.com/ContinuumIO/pycosat/master/examples/sudoku.pyd

import pycosat
from pprint import pprint


def translate_variable(lay, row, col, dig):
    """
...
def solve3d(grid):
    ...
    for lay in range(1, 10):
        for row in range(1, 10):
            for col in range(1, 10):
                ...
                # constraint
                if dig > 0:
                    clauses.append([translate_variable(lay, row, col, dig)])

    # Executes the SAT solver
    sol = pycosat.solve(clauses)
    sol = set(val for val in sol if val > 0)
    ...
    def read_cell(lay, row, col):
        ...
        for dig in range(1, 10):
            if translate_variable(lay, row, col, dig) in sol:
                return dig
...
if __name__ == '__main__':
    ...
    i = 1
    for w, wi in enumerate([(i, j) for j in range(1, 10)]):
        print w
        print wi
